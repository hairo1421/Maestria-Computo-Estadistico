...
**TAREA 5: SIMULACION COLAS CON ALEATORIOS DEL 1 AL 4
...
#include <time.h>
#include <math.h>
typedef struct { /*Declarando estructura con campo intt*/
int num;
} QueueData;
...
{	
	...
	for( t = 1; t <= 720; t++){	
		clientes = (clientes + (1 + ( rand() % 4 )));/*dato aleatorio*/
		if(clientes <= 720){
...
int *service()
{
   ...
   /*Se crea aquí el array dado a que no se sabía el espacio de memoria que se utilizaríá*/
	...
	for( t = 1; t <= contar; t++){/*Despachar  tantos clientes cuenten*/
		servicios = 0;
		servicios = (1 + ( rand() % 4 ));/*cuanto duran en atender a los clientes que van a llegar*/
...
int simulation()
{
	...
	int countNodo = 0;int  max[300];int j= 0;int aux =0;
